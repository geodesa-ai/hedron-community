...
Images are published at [`ghcr.io/geodesa-ai/hedron`](https://github.com/geodesa-ai/hedron/pkgs/container/hedron). Every image contains all supported model architectures and the production kernel set; tags select GPU machine code and CUDA runtime, not model families. Architecture-specific tags use the host platform of the corresponding hardware: discrete GPU targets are `linux/amd64`, while Jetson and GB10 targets are `linux/arm64`. Automatic tags are multi-platform manifests, so Docker selects the correct host image before Hedron selects the visible GPU target.
...
| `sm_87` | Ampere: Jetson AGX Orin, Orin NX, Orin Nano | `linux/arm64` | [`sm87-cu12`](https://github.com/geodesa-ai/hedron/pkgs/container/hedron?tag=sm87-cu12) | [`sm87-cu13`](https://github.com/geodesa-ai/hedron/pkgs/container/hedron?tag=sm87-cu13) |
...
| `sm_110f` | Blackwell: Jetson T5000/T4000 | `linux/arm64` | — | [`sm110f-cu13`](https://github.com/geodesa-ai/hedron/pkgs/container/hedron?tag=sm110f-cu13) |
...
| `sm_121f` | Blackwell: GB10, DGX Spark | `linux/arm64` | [`sm121f-cu12`](https://github.com/geodesa-ai/hedron/pkgs/container/hedron?tag=sm121f-cu12) | [`sm121f-cu13`](https://github.com/geodesa-ai/hedron/pkgs/container/hedron?tag=sm121f-cu13) |
...
Architecture-specific images are smaller and are the recommended production choice. Choose CUDA 12 when the host driver cannot load a CUDA 13 runtime; the CUDA toolkit installed on the host does not affect containers. CUDA 12.9 predates SM110, so that target is CUDA 13 only. On `linux/amd64`, automatic images package the discrete GPU targets available for their CUDA line. On `linux/arm64`, CUDA 13 packages SM87, SM110, and SM121; CUDA 12 packages SM87 and SM121. Automatic selection uses the Tegra device tree on Jetson and `nvidia-smi` elsewhere. If GPUs with different compute capabilities are visible in one container, set an explicit target such as `-e HEDRON_CUDA_TARGET=sm89` or expose only one GPU generation.
